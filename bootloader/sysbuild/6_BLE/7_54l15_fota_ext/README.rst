Sample with MCUBoot, BLE FOTA, KMU, custom private key, and external flash for 54l15
###################
...
and use BLE as the transport for a FOTA update. The images are signed with a custom private key and public key is stored in the KMU.
...
We want to use the Key Management Unit on the device to manage our keys. To do so enable ``SB_CONFIG_MCUBOOT_SIGNATURE_USING_KMU=y``per the mentioned docs
...
3. nrfutil erase --all 
3. Provision the KMU with the derived public key for MCUboot and NSIB with https://docs.nordicsemi.com/bundle/ncs-latest/page/nrf/app_dev/device_guides/nrf54l/kmu_provision.html#provisioning_keys_to_the_board.
